...
def test_check_cites_article_32(eurlex):
    ...
    assert "DMARC assente" in out.output
    assert "Norma applicata: GDPR art. 32\n" in out.output
...
def test_check_shows_evidence(eurlex):
    out = _check(_report())
    assert "nessun record DMARC" in out.output
    assert "MTA-STS assente" in out.output
...
def test_check_offline_without_cache():
    ...
    assert "Norma applicata: GDPR art. 32" in out.output
